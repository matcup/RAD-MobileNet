...
train_dataset = SealDataset(root_dir=str(train_path), transform=transform, feature_=feature_)
val_dataset = SealDataset(root_dir=str(val_path), transform=transform, feature_=feature_)
test_dataset = SealDataset(root_dir=str(test_path), transform=transform, feature_=feature_)
...
for epoch in range(num_epochs):
    ...
    # 计算训练集的指标
    train_loss, train_precision, train_recall, train_f1, _ = evaluate(model, train_loader)

    # 计算验证集的指标
    val_loss, val_precision, val_recall, val_f1, _ = evaluate(model, val_loader)
...
# 在测试集上评估模型
test_loss, test_precision, test_recall, test_f1, cm = evaluate(model, test_loader)
